...
# ============================================================
# FMSAM: Full Module (Modified, WITHOUT MCAU)
# ============================================================
class FMSAM(nn.Module):
    def __init__(self, channels):
        super().__init__()
        self.ffn = FFNBlock(channels)
        # MCAU removed → use identity for attention branch
        self.ln_after_ffn = LayerNorm2d(channels)
        self.flm = FuzzyLogicModulation(channels)
        self.final_conv = nn.Conv2d(channels, channels, 3, padding=1)
    def forward(self, x):
        x_ffn = self.ffn(x)
        # Identity instead of MCAU
        x_att = self.ln_after_ffn(x_ffn)
...
class FuzzySwinViTEncoder(nn.Module):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, embed_dim=96, depths=2, output_dim=256):
        ...
        self.blocks = nn.Sequential(*[SwinBlockFuzzy(embed_dim) for _ in range(depths)])
...
# ------------------ Save model ------------------
state_path = "/content/fuzzy_swinvit_encoder_state_noMCAU.pt"
...
encoder_cpu.eval()
script_path = "/content/fuzzy_swinvit_encoder_scripted_noMCAU.pt"
try:
...
drive_state_path = os.path.join(drive_dir, "fuzzy_swinvit_encoder_state_noMCAU.pt")
