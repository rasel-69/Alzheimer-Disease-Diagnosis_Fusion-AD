...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        x = self.proj(x)                 # [B, C, H, W] where H=W=img_size/patch
        x = x.flatten(2).transpose(1,2)  # [B, N, C] where N=H*W
        return x


# ------------------ Fuzzy Gated Attention using FMSAM ------------------
class FuzzyGatedAttention(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.fmsam = FMSAM(dim) ### using FMSAM here

    def forward(self, x):
        """
        x: [B, N, C]  (Transformer tokens)
        """
        B, N, C = x.shape
        H = W = int(N ** 0.5)
...
# ------------------ Swin Transformer Block (with FuzzyGatedAttention + Mish MLP) ------------------
class SwinBlockFuzzy(nn.Module):
...
# ------------------ Fuzzy SwinViT Encoder ------------------
class FuzzySwinViTEncoder(nn.Module):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, embed_dim=96, depths=2, output_dim=256):
        ...
        self.blocks = nn.Sequential(
            *[SwinBlockFuzzy(embed_dim) for _ in range(depths)]
        )
...
def compute_prototypical_loss(encoder, sx, sy, qx, qy, n_way):
    # train-mode version (kept as-is for training step)
    encoder.train()
...
def compute_prototypical_loss_eval(encoder, sx, sy, qx, qy, n_way):
    # eval-mode version (no .train(), no grads)
    encoder.eval()
    support_embeddings = encoder(sx)
    query_embeddings = encoder(qx)
    prototypes = compute_prototypes(support_embeddings, sy, n_way)
...
def evaluate_on_val(encoder, val_data, n_way, k_shot, q_query, episodes=5):
    ...
    if len(allowed) < n_way:
        raise ValueError(f"Validation split has only {len(allowed)} classes with at least {k_shot+q_query} images; need >= {n_way}.")
    losses, accs = [], []
...
# ------------------ Training Loop with Train/Val logging ------------------
encoder = FuzzySwinViTEncoder().to(DEVICE)
optimizer = torch.optim.Adam(encoder.parameters(), lr=1e-4)

train_data = load_dataset(train_dir)
val_data = load_dataset(val_dir)

# Ensure enough classes for episodes
allowed_train = get_allowed_classes(train_data, K_SHOT + Q_QUERY)
if len(allowed_train) < N_WAY:
    raise ValueError(f"Train split has only {len(allowed_train)} classes with at least {K_SHOT+Q_QUERY} images; need >= {N_WAY}.")
...
for epoch in range(1, EPOCHS + 1):
    ...
    # Mixed precision training for speed/memory (safe for inference-style encoders)
    ...
    # Validation (average over several random episodes)
...
# ------------------ Save model (.pt) for deployment ------------------
# 1) Save state_dict (recommended for PyTorch services)
state_path = "/content/fuzzy_swinvit_encoder_state.pt"
torch.save(encoder.state_dict(), state_path)
print(f"Saved state_dict to: {state_path}")

# 2) TorchScript export (good for serving in non-Python runtimes)
encoder_cpu = FuzzySwinViTEncoder().to("cpu")
encoder_cpu.load_state_dict(torch.load(state_path, map_location="cpu"))
encoder_cpu.eval()
script_path = "/content/fuzzy_swinvit_encoder_scripted.pt"
try:
...
drive_state_path = os.path.join(drive_dir, "fuzzy_swinvit_encoder_state.pt")
